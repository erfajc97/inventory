...
import { editEquipmentsThunk } from "../../../../store/inventory/thunks";

const useUpdateErrosEquipment = ({ id, closeEdit }) => {
  ...
  const onSubmitEdit = async (values) => {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      const response = await dispatch(editEquipmentsThunk(id, equipment));
      setLoading(false);
      form.resetFields();
...
export default useUpdateErrosEquipment;
